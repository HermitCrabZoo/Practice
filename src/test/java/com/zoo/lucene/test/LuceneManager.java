...
//	private Path source=Resource.onClassPath("source");
	private Path store=Resource.onClassPath("store");
...
//		Query query = NumericRangeQuery.newLongRange("fileSize", 47L, 200L, false, true);
		TermRangeQuery query =TermRangeQuery.newStringRange("fileSize", "47", "200", false, true);
		System.out.println(query);
		printResult(indexSearcher, query);
		//关闭资源
		indexSearcher.getIndexReader().close();
